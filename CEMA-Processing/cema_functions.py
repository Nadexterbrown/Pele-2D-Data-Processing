...
def simplified_cema(data, input_params):
    def cema_terms():
        ###########################################
        # Main Function
        #
    ...
    """
    def cantera_jacobian():
        # Step 1: Allocate memory for the jacobian array
        J = np.full((y_pts, x_pts, len(input_params.species), len(input_params.species)), np.nan)

        # Step 1: Create a temporary gas object
        gas_tmp = ct.Solution(input_params.mech)
        ...
        for j in range(0, y_pts):
            for i in range(0, x_pts):
                ...
                # Set the gas object state
                gas_tmp.TPY = (data['Temperature'][j, i],
                               data['Pressure'][j, i],
                               {species: data[f'Y({species})'][j, i]  # Construct key dynamically
                                for species in input_params.species  # Ensure key exists
                                })
                ...
                for row, primary_species in enumerate(input_params.species):
                    for col, secondary_species in enumerate(input_params.species):
                        ...
                        if row < tmp_jacobian.shape[0] and col < tmp_jacobian.shape[1]:
                            ...
                            tmp_jacobian[row, col] = (sum(temperature_jacobian[species] *
                                                         gas_tmp.molecular_weights[gas_tmp.species_index(species)] *
                                                         species_enthalpy[f'Y({species})'][j, i] for species in input_params.species) /
                                                      data['Density'][j, i] / data['Cp'][j, i])
    ...
    print("     Calculating Jacobian...")
    J = jacobian(data, input_params, chem_method='Cantera', jac_method='Simple')
    ...
    for j in range(y_pts):
        for i in range(x_pts):
            ...
            eig_idx = np.argmax(local_val[valid_mask])
            if abs(local_val[valid_mask][eig_idx]) < 1e-8:
                continue
            ...
            eig_idx = np.argmax(local_val)
            lambda_e[j, i] = local_val[eig_idx]
            b_e = local_vec[:, eig_idx].real
...
def compressible_cema(data, input_params, jacobian_type='simple'):
    def cema_terms():
        ###########################################
        # Main Function
        #
        ...
        def diffusion_terms():
            ...
            dtau_1 = spatial_derivative(tau_xx, data['Grid'][:, :, 0], 'x') + spatial_derivative(tau_xy, data['Grid'][:, :, 1], 'y')
            dtau_2 = spatial_derivative(tau_yx, data['Grid'][:, :, 0], 'x') + spatial_derivative(tau_yy, data['Grid'][:, :, 1], 'y')
            ...
            for j in range(0, y_pts):
                for i in range(0, x_pts):
                    ...
                    for idx, species in enumerate(input_params.species):
                        g_d[j, i, idx] = dJdx[f'Y({species})'][j, i] + dJdy[f'Y({species})'][j, i]
                        g_r[j, i, idx] = - data['Density'][j, i] * data[f'Y({species})'][j, i] * (dudx[j, i] + dvdy[j, i])
                        if not np.isnan(g_r[j, i, idx]):
                            g_T[j, i, idx] = 0
    ...
    def cantera_jacobian():
        ...
        for j in range(0, y_pts):
            for i in range(0, x_pts):
                ...
                # Map the jacobians to a pandas dataframe
                species_jacobian = pd.DataFrame(species_jacobian,
                                                index=gas_tmp.species_names,
                                                columns=gas_tmp.species_names)
                temperature_jacobian = pd.Series(temperature_jacobian,
                                                 index=gas_tmp.species_names)
                ...
                for row in range(0, tmp_jacobian.shape[0]):
                    ...
                    for col in range(0, tmp_jacobian.shape[1]):
                        ...
                        elif row < len(input_params.species) and col == len(input_params.species) + 1 and data['Dimension'] == 2:
                            tmp_jacobian[row, col] = (- (data['Y Velocity'][j, i] *
                                                         gas_tmp.molecular_weights[
                                                             gas_tmp.species_index(primary_species)] /
                                                         data['Density'][j, i] / data['Cv'][j, i]) *
                ...
                for row in range(0, tmp_jacobian.shape[0]):
                    if row < len(input_params.species):
                        primary_species = input_params.species[row]
                        #secondary_species = input_params.species[row]
                    else:
                        tmp_jacobian[row, col] = 0
                        continue
                    ...
                    for col in range(0, tmp_jacobian.shape[1]):
                        if col < len(input_params.species):
                            #primary_species = input_params.species[col]
                            secondary_species = input_params.species[col]
                        ...
                        if row < len(input_params.species):
                            tmp_jacobian[row, col] = ((gas_tmp.molecular_weights[gas_tmp.species_index(primary_species)] /
                                                      ...
                                                      species_jacobian[primary_species][secondary_species] +
                                                      (gas_tmp.molecular_weights[gas_tmp.species_index(primary_species)] /
                                                       data['Density'][j, i] * data['Cv'][j, i]) *
                                                      (data['X Velocity'][j, i] ** 2 / 2 -
                                                       (gas_tmp.standard_int_energies_RT[gas_tmp.species_index(secondary_species)] *
                                                        ct.gas_constant * data['Temperature'][j, i]) /
                                                       gas_tmp.molecular_weights[gas_tmp.species_index(secondary_species)])
                                                      ) * temperature_jacobian[primary_species]
                        ...
                        if data['Dimension'] == 2:
                            if row == len(input_params.species) + 1:
                                tmp_jacobian[row, col] = (- (data['Y Velocity'][j, i] *
                                                             gas_tmp.molecular_weights[
                                                                 gas_tmp.species_index(primary_species)] /
                                                             data['Density'][j, i] / data['Cv'][j, i]) *
                        ...
                        if row == len(input_params.species) + data['Dimension']:
                            tmp_jacobian[row, col] = ((gas_tmp.molecular_weights[gas_tmp.species_index(primary_species)] /
                                                      data['Density'][j, i] / data['Cv'][j, i]) *
                                                      temperature_jacobian[primary_species])
    ...
    # Step 2: Determine the source term vectors
    print("     Calculating CEMA Terms...")
    g_w, g_d, g_r, g_T = cema_terms()

    # Step 3: Calculate the Jacobian
    print("     Calculating Jacobian...")
    J = jacobian(data, input_params, chem_method='Cantera', jac_method='Compressible')

    # Step 4: Determine the eigenstate at each point
    print("     Calculating Eigenvalues...")
    eig_val = np.zeros((y_pts, x_pts, len(input_params.species) + (data['Dimension'] + 1)), dtype=np.complex128)
    eig_vec = np.zeros((y_pts, x_pts, len(input_params.species) + (data['Dimension'] + 1), len(input_params.species) + (data['Dimension'] + 1)),
                       dtype=np.complex128)
    ...
    lambda_e = np.full((y_pts, x_pts), np.nan)
    phi_w = np.full((y_pts, x_pts), np.nan)
    phi_d = np.full((y_pts, x_pts), np.nan)
    phi_r = np.full((y_pts, x_pts), np.nan)
    phi_T = np.full((y_pts, x_pts), np.nan)
