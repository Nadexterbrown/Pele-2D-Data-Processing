...
__all__ = [
    ...
    'domain_parameters', 'data_extraction', 'wave_tracking'
]
...
def data_extraction(data_dir, extract_location, direction='x', raw_data=None, preloaded_grids=None):
    ##########################################
    # Internal Functions
    ###########################################
...
def wave_tracking(wave_type, **kwargs):
    ...
    def find_wave_index(wave_type):
        if wave_type == 'Flame':
            ...
            return np.argwhere(data_arr >= 1.01 * data_arr[-1])[-1]
        else:
